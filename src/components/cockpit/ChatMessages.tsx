      ...
      <ScrollArea className="h-full" ref={scrollRef} onScrollCapture={handleScroll}>
        <div className="p-4 space-y-3">
          {messages.map((msg, idx) => {
            ...
            const visualSearchMediaId = displayContent ? extractVisualSearchMediaId(displayContent) : null;
            const hasVisualSearch = displayContent ? isVisualSearchContent(displayContent) : false;

            // Check if this is an agent entry system message or system source
            const isSystemMessage = msg.source === "system" || (displayContent ? isAgentEntryMessage(displayContent, msg.direction) : false);
            ...
            if (!displayContent && !hasImage) {
              ...
              if (showDateSeparator) {
                return (
                  <div key={msg.id || msg._id || idx} className="flex justify-center my-4">
                ...
                {isSystemMessage ? (
                  ...
                  <div className={cn("flex", isInbound ? "justify-start" : "justify-end")}>
                    ...
                    >
                      ...
                      {displayContent && !hasVisualSearch && (
                        <p className="whitespace-pre-wrap break-words">{cleanContent(displayContent)}</p>
                      )}
                      {time && (
                        <div className={cn(
